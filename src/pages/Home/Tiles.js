    ...
    t.push(
      <span
        ...
        onClick={() => {
          ...
          const isTileAlreadyClicked = clonedUserAnswer.find(
            (x) => x.tile_pos === tilePos
          );
          ...
          const isTileAlreadyCorrect = (() => {
            ...
            correctAnswers.every((x) => {
              if (!x.word.includes(combinedString[i])) {
                ...
                const found = x.word_tile_positions.find(
                  (w) => w.tile_pos === tilePos
                );
                if (found) {
                  isCorrect = true;
                  return false;
          ...
          if (
            userAnswer.length <= 5 &&
            !isTileAlreadyClicked &&
            !isTileAlreadyCorrect
          ) {
